...
void AddJEC_ForNanoV15(std::string inputFilePath, std::string outputFilePath, std::string jecVersion){
  std::cout << "=========================================================" << std::endl;
  std::cout << "AddJEC_ForNanoV15()::Start" << std::endl;
  ...
  TTree* inTree = inputTreeFile.Get<TTree>("Events");
  ...
  TTreeReaderArray<Float_t> Jet_muonSubtrDeltaEta = {reader, "Jet_muonSubtrDeltaEta"};
  TTreeReaderArray<Float_t> Jet_muonSubtrDeltaPhi = {reader, "Jet_muonSubtrDeltaPhi"};
  ...
  TTreeReaderArray<Float_t> CorrT1METJet_muonSubtrDeltaEta = {reader, "CorrT1METJet_muonSubtrDeltaEta"};
  TTreeReaderArray<Float_t> CorrT1METJet_muonSubtrDeltaPhi = {reader, "CorrT1METJet_muonSubtrDeltaPhi"};
  ...
  for(uint iEntry=0; iEntry < nEntries; iEntry++){
    ...
    for (int iJet=0; iJet < *nJet; iJet++){
      ...
      float jec = recojet_pt / recojet_pt_raw;
      ...
      float recojet_eta_noMuRaw  = Jet_muonSubtrDeltaEta[iJet] + recojet_eta;
      float recojet_phi_noMuRaw  = Jet_muonSubtrDeltaPhi[iJet] + recojet_phi;
    ...
    for (int iJet=0; iJet < *nCorrT1METJet; iJet++){
      ...
      //
      // 1. Get the jet raw p4, without muons included
      //
      float recojet_pt_noMuRaw = recojet_pt_raw * (1.f - recojet_muonSubtrFactor);
      float recojet_mass_noMuRaw = recojet_mass_raw * (1.f - recojet_muonSubtrFactor);
      float recojet_eta_noMuRaw = CorrT1METJet_muonSubtrDeltaEta[iJet] + recojet_eta;
      float recojet_phi_noMuRaw = CorrT1METJet_muonSubtrDeltaPhi[iJet] + recojet_phi;
  ...
  inputTreeFile.Close();
  std::cout << "AddJEC_ForNanoV15()::Finished" << std::endl;
}
